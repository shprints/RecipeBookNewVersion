...
<main role="main">
    ...
    <#if known>
     <#if language=="Russian">
      <h1 class="display-3">Здравствуй, ${name}!</h1>
     <p class="lead my-3"><em>
         Добро пожаловть в книгу рецептов! Это уникальный форум, где вы можете найти любой рецепт и порадовать своих друзей и близких кулинарным мастерством, а также поделиться своими секретами с другими участниками форума. Желаю отлично провести время!
     </em></p>
    ...
    <#if known>
    <#if language=="Russian">
        <h1><i>Список рецептов</i></h1>
    <#else>
        <h1><i>List of recipes</i></h1>
    </#if>
        <footer class="pt-4 my-md5 pt-md-3 border-top"></footer>

    <#if language=="Russian">
        <p class="lead my-3"><em>Секреты кулинарного мастерства здесь:</em></p>
    <#else>
    <p class="lead my-3"><em>Secrets of culinary excellence here:</em></p>
    </#if>
    <#else>
    <h1><i>List of recipes</i></h1>
    <footer class="pt-4 my-md5 pt-md-3 border-top"></footer>
    ...
    <#if language=="Russian">
        ...
        <p class="mb-0"><em>“Открытие нового блюда делает больше для счастья человеческой расы, чем открытие звезды.”
        </em></p>
